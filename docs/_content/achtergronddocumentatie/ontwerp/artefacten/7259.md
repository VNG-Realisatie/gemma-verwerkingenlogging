...
-	De partijen zijn zo min mogelijk afhankelijk van elkaar bij het implementeren van de logging. Log en VAR van het RDW staan immers los van het log en VAR van de gemeente, zowel tijdens de registratie als bij latere raadpleging.
...
Ook de gevolgen zijn identiek. De systemen zijn zo onafhankelijk mogelijk in hun implementatie van logging. En ook de granulariteit zal verschillen. Het gegevensmagazijn zal bijvoorbeeld `Gegevensverstrekking t.b.v. burgerzaken` loggen terwijl het burgerzakensysteem `Spoedprocedure rijbewijs` logt.
