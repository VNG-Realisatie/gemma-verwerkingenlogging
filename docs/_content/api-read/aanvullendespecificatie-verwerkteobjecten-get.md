...
* Zowel de verwerkingsacties als de verwerkte objecten die in deze verwerkingsacties zijn opgenomen worden gefilterd op basis van query parameters `objecttype`, `soortObjectId` en `objectId`. Voorbeeld: we zoeken naar de verwerkingsacties met betrekking tot de persoon met BSN 8273365. Dit is een GET operatie met de volgende query parameters:
  - `objecttype=Persoon`,
  - `soortObjectId=BSN`,
  - `objectId=8273365`. 
...
* Als het log meerdere versies van een verwerkingsactie bevat dan worden alleen actuele en niet vervallen versies geretourneerd. Zie [B3891](../achtergronddocumentatie/ontwerp/artefacten/3891.md) voor meer informatie over versies van verwerkingsacties.
